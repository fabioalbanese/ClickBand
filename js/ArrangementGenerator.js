...
(function(global){
  ...
  ArrangementGenerator.prototype.arrange=function(baseSong){
    this.validateBase(baseSong);var snapshot=JSON.stringify(baseSong);
  ...
  ArrangementGenerator.prototype.arrangeBar=function(sectionName,phrase,bar){
    ...
    var root=chord.tones[0],fifth=chord.tones[2],voices={},i;
    if(enabled(this,"arp")){voices.arp=[];for(i=0;i<8;i++){var t=chord.tones[i%3];voices.arp.push(note(t.degree,t.octaveOffset+1,i*2,2,sectionName==="intro"?0.38:0.46));}}
    if(enabled(this,"bass")){voices.bass=[];for(i=0;i<4;i++){var bt=i===3?fifth:root;voices.bass.push(note(bt.degree,bt.octaveOffset-2,i*4,4,sectionName==="chorus"?0.64:0.58));}}
    if(enabled(this,"guitar")){voices.guitar=[];for(i=0;i<4;i++)voices.guitar.push(chordEvent(chord,i*4,3,sectionName==="chorus"?0.46:0.38,-1));}
    if(enabled(this,"chromatic")&&(sectionName==="chorus"||sectionName==="bridge")){
      voices.chromatic=[
    ...
    if(enabled(this,"pad"))voices.pad=[chordEvent(chord,0,15,0.32,0)];
    if(enabled(this,"ostinato")){voices.ostinato=[];for(i=0;i<8;i++){var ot=chord.tones[(i%4<2)?0:2];voices.ostinato.push(note(ot.degree,ot.octaveOffset+1,i*2,2,0.36));}}
    if(enabled(this,"counter")&&(sectionName==="chorus"||sectionName==="bridge")){var ct=chord.tones[1];voices.counter=[note(ct.degree,ct.octaveOffset+1,10,3,0.40)];}
    if(enabled(this,"fx")&&bar===0){var ft=chord.tones[0];voices.fx=[note(ft.degree,ft.octaveOffset+2,0,2,0.36)];}
    if(enabled(this,"choir")&&(sectionName==="chorus"||sectionName==="bridge"||sectionName==="outro"))voices.choir=melody.map(function(n){return note(n.degree,n.octaveOffset+1,n.startSpot,n.durationSpots,sectionName==="chorus"?0.42:0.35,{accidental:n.accidental,articulation:"legato",source:"melody-double"});});
    if(enabled(this,"brass")&&sectionName==="chorus")voices.brass=[chordEvent(chord,0,2,0.42,0)];
    if(enabled(this,"strings")&&sectionName==="chorus"){voices.strings=[];[2,6,10,14].forEach(function(st){voices.strings.push(chordEvent(chord,st,2,0.33,0));});}
