...
(function(global){
  ...
  ArrangementGenerator.prototype.arrangeBar=function(sectionName,phrase,bar){
    ...
    var root=chord.tones[0],fifth=chord.tones[2],voices={},i,isFolk=this.style==="FOLK",isLatin=this.style==="LATIN",isJazz=this.style==="JAZZ",isBlues=this.style==="BLUES",isCeltic=this.style==="CELTIC";
    if(enabled(this,"arp")){voices.arp=[];for(i=0;i<8;i++){var t=isFolk?chord.tones[[0,1,2,1,0,1,2,1][i]]:(isLatin?chord.tones[[0,1,2,1,2,1,0,1][i]]:(isJazz?chord.tones[[0,2,1,2,0,1,2,1][i]]:(isBlues?chord.tones[[0,1,2,1,0,2,1,2][i]]:(isCeltic?chord.tones[[0,1,2,1,2,1,0,1][i]]:chord.tones[i%3]))));voices.arp.push(note(t.degree,t.octaveOffset+1,i*2,2,(isFolk?(sectionName==="intro"?0.34:0.40):(isLatin?(sectionName==="intro"?0.40:0.48):(isJazz?(sectionName==="intro"?0.32:0.40):(isBlues?(sectionName==="intro"?0.30:0.38):(isCeltic?(sectionName==="intro"?0.38:0.46):(sectionName==="intro"?0.38:0.46)))))),isFolk?{articulation:"short",role:"acoustic-picking"}:(isLatin?{articulation:"short",role:"piano-montuno"}:(isJazz?{articulation:"legato",role:"jazz-piano-voicing"}:(isBlues?{articulation:"short",role:"blues-organ-riff"}:(isCeltic?{articulation:"short",role:"fiddle-reel-bowing"}:null))))));}}
    if(enabled(this,"bass")){voices.bass=[];for(i=0;i<4;i++){var bt=isFolk?(i%2===0?root:fifth):(isLatin?([0,3].indexOf(i)!==-1?root:fifth):(isJazz?chord.tones[[0,1,2,1][i]]:(isBlues?(i%2===0?root:fifth):(isCeltic?(i%2===0?root:fifth):(i===3?fifth:root)))));voices.bass.push(note(bt.degree,bt.octaveOffset-2,i*4,4,sectionName==="chorus"?0.64:0.58,isFolk?{articulation:"short",role:"root-fifth"}:(isLatin?{articulation:"short",role:"tumbao-bass"}:(isJazz?{articulation:"short",role:"walking-bass"}:(isBlues?{articulation:"short",role:"blues-shuffle-bass"}:(isCeltic?{articulation:"short",role:"celtic-root-fifth"}:null))))));}}
    if(enabled(this,"guitar")){voices.guitar=[];if(isFolk){voices.guitar.push(note(root.degree,root.octaveOffset-1,0,3,0.42,{articulation:"short",role:"boom"}));voices.guitar.push(chordEvent(chord,4,2,0.40,-1));voices.guitar.push(note(fifth.degree,fifth.octaveOffset-1,8,3,0.42,{articulation:"short",role:"boom"}));voices.guitar.push(chordEvent(chord,12,2,sectionName==="chorus"?0.48:0.42,-1));}else if(isLatin){[2,6,10,14].forEach(function(st){var ev=chordEvent(chord,st,2,sectionName==="chorus"?0.50:0.43,-1);ev.role="salsa-comping";ev.articulation="short";voices.guitar.push(ev);}); }else if(isJazz){[0,6,10].forEach(function(st){var ev=chordEvent(chord,st,2,sectionName==="chorus"?0.38:0.32,-1);ev.role="freddie-green-comping";ev.articulation="short";voices.guitar.push(ev);});}else if(isBlues){[0,3,6,8,11,14].forEach(function(st){var ev=chordEvent(chord,st,2,sectionName==="chorus"?0.46:0.39,-1);ev.role="blues-shuffle-guitar";ev.articulation="short";voices.guitar.push(ev);});}else if(isCeltic){[0,4,8,12].forEach(function(st){var ev=chordEvent(chord,st,3,sectionName==="chorus"?0.45:0.38,-1);ev.role="celtic-strum";ev.articulation="short";voices.guitar.push(ev);});}else{for(i=0;i<4;i++)voices.guitar.push(chordEvent(chord,i*4,3,sectionName==="chorus"?0.46:0.38,-1));}}
    ...
    if(enabled(this,"pad")&&!isFolk&&!isLatin&&!isJazz&&!isCeltic)voices.pad=[chordEvent(chord,0,15,0.32,0)];
    if(enabled(this,"ostinato")){voices.ostinato=[];for(i=0;i<8;i++){var ot=isFolk?chord.tones[[0,1,2,1,2,1,0,2][i]]:(isLatin?chord.tones[[0,2,1,2,0,2,1,2][i]]:(isJazz?chord.tones[[0,1,2,1,0,2,1,2][i]]:(isBlues?chord.tones[[0,2,1,2,0,1,2,1][i]]:(isCeltic?chord.tones[[0,1,2,1,0,1,2,1][i]]:chord.tones[(i%4<2)?0:2]))));voices.ostinato.push(note(ot.degree,ot.octaveOffset+1,i*2,2,isFolk?0.42:(isLatin?0.44:(isJazz?0.30:(isBlues?0.32:(isCeltic?0.40:0.36)))),isFolk?{articulation:"short",role:"banjo-roll"}:(isLatin?{articulation:"short",role:"marimba-cascara"}:(isJazz?{articulation:"legato",role:"vibraphone-comping"}:(isBlues?{articulation:"short",role:"blues-piano-riff"}:(isCeltic?{articulation:"short",role:"celtic-harp-ostinato"}:null))))));}}
    if(enabled(this,"counter")&&(sectionName==="chorus"||sectionName==="bridge")){var ct=chord.tones[1];voices.counter=isFolk?[note(ct.degree,ct.octaveOffset+1,8,2,0.38,{articulation:"short",role:"harmonica-answer"}),note(fifth.degree,fifth.octaveOffset+1,12,3,0.42,{role:"harmonica-answer"})]:(isLatin?[note(ct.degree,ct.octaveOffset+1,6,2,0.40,{articulation:"short",role:"flute-response"}),note(fifth.degree,fifth.octaveOffset+1,14,2,0.44,{articulation:"short",role:"flute-response"})]:(isJazz?[note(ct.degree,ct.octaveOffset+1,7,3,0.34,{articulation:"legato",role:"vibes-response"}),note(fifth.degree,fifth.octaveOffset+1,13,3,0.38,{articulation:"legato",role:"vibes-response"})]:(isBlues?[note(ct.degree,ct.octaveOffset+1,6,3,0.38,{articulation:"bend",role:"blues-call-response"}),note(fifth.degree,fifth.octaveOffset+1,12,3,0.42,{role:"blues-call-response"})]:(isCeltic?[note(ct.degree,ct.octaveOffset+1,4,2,0.38,{articulation:"short",role:"tin-whistle-response"}),note(fifth.degree,fifth.octaveOffset+1,12,2,0.42,{articulation:"short",role:"tin-whistle-response"})]:[note(ct.degree,ct.octaveOffset+1,10,3,0.40)]))));}
    ...
    if(!isFolk&&enabled(this,"brass")&&sectionName==="chorus"){voices.brass=isLatin?[chordEvent(chord,0,2,0.50,0),chordEvent(chord,6,2,0.46,0),chordEvent(chord,12,2,0.52,0)]:(isJazz?[chordEvent(chord,4,2,0.34,0),chordEvent(chord,12,2,0.38,0)]:[chordEvent(chord,0,2,0.42,0)]);if(isLatin)voices.brass.forEach(function(ev){ev.role="mambo-brass";ev.articulation="short";});if(isJazz)voices.brass.forEach(function(ev){ev.role="muted-brass-answer";ev.articulation="short";});}
    if(enabled(this,"strings")&&sectionName==="chorus"){voices.strings=[];if(isFolk){[0,2,4,6,8,10,12,14].forEach(function(st){voices.strings.push(note((st%4===0?root:fifth).degree,(st%4===0?root:fifth).octaveOffset+1,st,2,0.38,{articulation:"short",role:"fiddle-sawstroke"}));});}else if(isCeltic){[0,2,4,6,8,10,12,14].forEach(function(st){voices.strings.push(note((st%4===0?root:fifth).degree,(st%4===0?root:fifth).octaveOffset+1,st,2,0.42,{articulation:"short",role:"fiddle-reel"}));});}else{[2,6,10,14].forEach(function(st){voices.strings.push(chordEvent(chord,st,2,0.33,0));});}}
