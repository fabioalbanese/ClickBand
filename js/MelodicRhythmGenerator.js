...
(function (global) {
  ...
  function MelodicRhythmGenerator(config) {
    ...
    /*
     * Meaningful rests are internal phrase punctuation. They do not alter the
     * public output contract: a rest is represented by the absence of notes.
     */
    this.meaningfulRests = config.meaningfulRests !== false;
    this.restProbability = config.restProbability === undefined
      ? 0.28
      : Math.max(0, Math.min(1, Number(config.restProbability) || 0));
  ...
  MelodicRhythmGenerator.prototype.createMeaningfulRest = function (starts, barIndex) {
    var result = { starts: clone(starts), rest: null };

    /* Only the two central bars can contain phrase punctuation. */
    if (!this.meaningfulRests || (barIndex !== 1 && barIndex !== 2)) return result;

    /* A rest is useful only when the bar is genuinely dense. */
    if (result.starts.length < 5 || Math.random() >= this.restProbability) return result;

    var candidates = [], i, restStart, nextStart, previousStart, score;

    for (i = 1; i < result.starts.length - 1; i++) {
      restStart = result.starts[i];
      nextStart = result.starts[i + 1];
      previousStart = result.starts[i - 1];

      /* Keep a real silence of at least an eighth note. */
      if (nextStart - restStart < 2) continue;

      /* The preceding note must still have room for a valid minimum duration. */
      if (restStart - previousStart < 2) continue;

      /* Prefer a comma around beat 3, then a smaller break around beat 4. */
      score = restStart === 8 ? 0 : restStart === 12 ? 1 : Math.abs(restStart - 8) + 2;
      candidates.push({
        index: i,
        start: restStart,
        end: nextStart,
    ...
    /* Usually choose the strongest punctuation point; occasionally the second. */
    var poolSize = Math.min(2, candidates.length);
    var selected = candidates[Math.floor(Math.random() * poolSize)];

    result.starts.splice(selected.index, 1);
    result.rest = {
  ...
  MelodicRhythmGenerator.prototype.chooseDuration = function (start, nextStart, finalBar, closing, rest) {
    var naturalEnd = nextStart === undefined ? 16 : nextStart;
    var protectedEnd = naturalEnd;

    /* Do not allow the preceding note to fill a designed phrase rest. */
    if (rest && start < rest.start && naturalEnd > rest.start) {
      protectedEnd = rest.start;
    }

    var gap = protectedEnd - start;
  ...
  MelodicRhythmGenerator.prototype.toEvents = function (starts, finalBar, rest) {
    var events = [];
    for (var i = 0; i < starts.length; i++) {
      var closing = !!(finalBar && i === starts.length - 1);
      events.push({
        start: starts[i],
        duration: this.chooseDuration(starts[i], starts[i + 1], finalBar, closing, rest),
  ...
  MelodicRhythmGenerator.prototype.generate = function (sectionName, variant) {
    ...
    var attempt, startsByBar, restsByBar, used, bar, starts, restResult, closeLong, finalStarts, fullSignature;
    for (attempt = 0; attempt < 24; attempt++) {
      startsByBar = []; restsByBar = []; used = [];
      for (bar = 0; bar < 3; bar++) {
        starts = this.pickPattern(families, used, recentFirstBars, bar === 0);
        if (variant === "B") starts = this.makeBVariation(starts);

        restResult = this.createMeaningfulRest(starts, bar);
        starts = restResult.starts;

        startsByBar.push(starts);
        restsByBar.push(restResult.rest);
    ...
    return {
      ...
      bars: startsByBar.map(function (barStarts, index) {
        return this.toEvents(barStarts, index === 3, restsByBar[index]);
      }, this)
    };
  };
